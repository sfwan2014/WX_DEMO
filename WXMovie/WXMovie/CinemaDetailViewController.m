...
- (void)dealloc {
    ...
    WXRelease(_cinemaId);
    WXRelease(_cinemaView);
    WXRelease(_ticketData);
    WXRelease(_dateArray);
    [super dealloc];
}
...
- (void)viewDidLoad
{
    ...
    //加载数据
    [self performSelector:@selector(_loadCinemaData) withObject:nil afterDelay:1.5];
}
...
- (void)_loadCinemaData {
    ...
    NSDictionary *jsonData = [WXDataService requestData:cinema_detail];
    
    /*__________________1.解析影院数据________________________*/
    NSDictionary *cinemaDic = [jsonData objectForKey:@"cinema"];
    CinemaModel *cinemaModel = [[CinemaModel alloc] initContentWithDic:cinemaDic];
    ...
    for (NSDictionary *movieDic in movieList) {
        MovieListModel *mm = [[MovieListModel alloc] initContentWithDic:movieDic];
    ...
    for (NSDictionary *ticketDic in ticketUnitList) {
        ...
        for (int i=0; i<ticketList.count; i++) {
            ...
            //创建场次model
            TicketModel *ticketModel = [[TicketModel alloc] initContentWithDic:ticketDic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        cell = [[[TicketCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identify] autorelease];
    }
